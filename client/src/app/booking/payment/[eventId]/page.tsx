...
export default function PaymentPage() {
  ...
  const [selectedSeats, setSelectedSeats] = useState<QRCodeType[]>(sampleQRCodeData);
  ...
  return <div className="w-full min-h-screen py-6 px-12 flex flex-row">
    <div className="w-[70%] flex flex-col items-center gap-6">

      <div className="flex flex-col items-center gap-1">
        <TimeCount intialTime={600} onTimeEnd={handleTimeEnd} />
        <p className="font-semibold text-[16px] text-[#000]">Your tickets are guaranteed for this time!</p>
      </div>

      <div className="w-[80%] p-2 border-b border-t border-gray-400 flex flex-row items-center justify-center gap-2">
        <div onClick={() => handleChangePaymentState("delivery")} className={`px-3 py-1 cursor-pointer rounded-[6px] ${paymentState === "delivery" ? "bg-[#2ECC7166]" : "bg-[#6868681A]"}`}>
          <span className={`text-[16px] font-bold ${paymentState === "delivery" ? "text-black" : "text-gray-400"}`}>1. Delivery</span>
        </div>
        <ChevronRight color={`${paymentState === 'delivery' ? '#2ECC71' : '#99a1af'}`} className={`w-5 h-5 font-bold`} />
        <div onClick={() => handleChangePaymentState("payment")} className={`px-3 py-1 cursor-pointer rounded-[6px] ${paymentState === "payment" ? "bg-[#2ECC7166]" : "bg-[#6868681A]"}`}>
          <span className={`text-[16px] font-bold ${paymentState === "payment" ? "text-black" : "text-gray-400"}`}>2. Payment</span>
        </div>
        <ChevronRight color={`${paymentState === 'confirm' ? '#2ECC71' : '#99a1af'}`} className={`w-5 h-5 font-bold`} />
        <div onClick={() => handleChangePaymentState("confirm")} className={`px-3 py-1 cursor-pointer rounded-[6px] ${paymentState === "confirm" ? "bg-[#2ECC7166]" : "bg-[#6868681A]"}`}>
          <span className={`text-[16px] font-bold ${paymentState === "confirm" ? "text-black" : "text-gray-400"}`}>3. Confirm</span>
        </div>
      </div>
      ...
      <div className="w-[80%]">
          ...
          <div className="w-full flex flex-col items-start gap-6">
            <div className="w-full flex flex-col items-start gap-2">
              <div className="w-full flex flex-col items-start">
                ...
                <p className="text-[16px] font-semibold text-[#686868]">Where do you want your tickets and receipt emailed?</p>
              </div>
              <TextField onChange={(e) => {setFullName(e.target.value)}} placeholder="Full name"/>
              <div className="w-full flex flex-row gap-1">
                <TextField onChange={(e) => {setEmail(e.target.value)}} placeholder="Email"/>
                <TextField onChange={(e) => {setPhone(e.target.value)}} placeholder="Phone"/>
              </div>
            </div>
            <Button className="w-full py-5" onClick={handleDeliveryContinue}>Continue</Button>
          </div>
        )}
        ...
        {paymentState === "payment" && (
          <div className="w-full flex flex-col items-start gap-5">
            ...
            <div className="w-full flex flex-col gap-2">
              <TextField onChange={(e) => setCardNumber(e.target.value)} placeholder="Card number" />
              <TextField onChange={(e) => setCardHolder(e.target.value)} placeholder="Card holder" />
              <div className="w-full flex flex-row gap-2">
                <TextField onChange={(e) => setExpiryDate(e.target.value)} placeholder="MM / YY" />
                <TextField onChange={(e) => setCvv(e.target.value)} placeholder="CVV" />
              </div>
            </div>
          ...
          <div className="w-full flex flex-col items-start gap-5">
            ...
            <div className="w-full px-4 grid grid-cols-2 gap-4">
              {selectedSeats.map((ticket, index) => (
                <TicketCard date={ticket.date} eventName={ticket.eventName} key={index} row={ticket.row} section={ticket.section} seatNumber={ticket.seatNumber} time={ticket.time} venue={ticket.venue} />
              ))}
            </div>
        </div>
        )}
    ...
    <div className="w-[25%] rounded-lg p-2 flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-2">
        <div className="w-full flex flex-row justify-between items-center gap-1">
          ...
          <p className="text-lg font-bold">{sampleEvents[0].Name}</p>
        </div>
        ...
        <div className="w-full flex flex-row justify-between items-center">
          ...
          <p className="text-base font-semibold">{sampleEvents[0].VenueId}</p>
        </div>
        <div className="w-full flex flex-row justify-between items-center">
          <p className="text-sm text-gray-500">Time</p>
          <p className="text-base font-semibold">{sampleEvents[0].StartDateTime}</p>
        </div>
      </div>
    ...
    {isOpenInfoTimePopup && (<TimeInfoConfirmPopup minutes={10} onStart={() => setIsOpenInfoTimePopup(false)} />)}
  </div>;
}
